...
examples how the more advanced functions in \autoref{sec:universal}
and \autoref{sec:advanced} can be used. Just type the name of such a
function to display its source.
<<>>=
urnorm
@ 
