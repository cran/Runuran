...
d <- unuran.discr.new(pv=dbinom(0:100,100,0.4), name="binomial(100,0.4)")
...
d <- unuran.discr.new(pmf=f,lb=0,mode=0,sum=1)
...
lf  <- function (x) { -sqrt(1+x^2) }
d <- unuran.cont.new(pdf=lf,islog=TRUE)
